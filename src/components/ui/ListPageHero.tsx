...
interface ListPageHeroProps {
  ...
  video?: string;
  minHeight?: string;
  /** "Resources", or a trail: [{ name: "Resources", href: "..." }, { name: "Post" }] */
  breadcrumb?: string | Crumb[];
  /**
   * Heading level for the banner title. Detail pages should pass "p" so the
   * article's own title stays the page's single <h1>.
   */
  titleAs?: "h1" | "p";
}
    ...
    <section className={`-mt-[76px] relative ${minHeight} flex items-end`}>
      ...
      <div className="relative w-full site-container px-6 pt-[76px] pb-16">
        <div className="flex items-end justify-between gap-8">
          ...
          >
            {breadcrumb && <Breadcrumb trail={breadcrumb} className="mb-3" />}
            <TitleTag className="text-2xl sm:text-3xl md:text-4xl font-bold text-white whitespace-nowrap">
              {title}
            </TitleTag>
          </motion.div>
